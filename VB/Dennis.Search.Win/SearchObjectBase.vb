...
Namespace Dennis.Search.Win
    ...
    Public MustInherit Class SearchObjectBase(Of T As IXPSimpleObject)
        ...
        Public Property Criteria As GroupOperator Implements ISearchObject.Criteria
            Get
                If ReferenceEquals(criteriaCore, Nothing) Then criteriaCore = New GroupOperator(GroupCriteriaType, EmptyCollectionCriteria)
                Return criteriaCore
            End Get
        ...
        Protected Overrides Sub OnChanged(ByVal propertyName As String, ByVal oldValue As Object, ByVal newValue As Object)
            ...
            If Not IsLoading AndAlso Not IsSaving Then
                If Not Equals(propertyName, "Criteria") AndAlso Not Equals(propertyName, SearchResultsCollectionName) Then UpdateCriteria(propertyName, newValue)
            End If
        End Sub
        ...
        Protected Overridable Sub UpdateCriteria(ByVal propertyName As String, ByVal propertyValue As Object)
            ...
            For Each operand As CriteriaOperator In Criteria.Operands
                ...
                If Not ReferenceEquals(binaryOperator, Nothing) AndAlso Not ReferenceEquals(binaryOperator, EmptyCollectionCriteria) Then
                    If Equals(CType(binaryOperator.LeftOperand, OperandProperty).PropertyName, propertyName) Then
        ...
        Public ReadOnly Property SearchResultsProp As XPCollection(Of T)
            Get
                If searchResultsCore Is Nothing Then searchResultsCore = New XPCollection(Of T)(Session, EmptyCollectionCriteria, Nothing)
                Return searchResultsCore
            End Get
        End Property
        ...
        Private ReadOnly Property SearchResults As ICollection Implements ISearchObject.SearchResults
            Get
                Return SearchResultsProp
            End Get
        End Property

        Public Overridable Sub Search() Implements ISearchObject.Search
            isSearchingCore = True
            If Criteria.Operands.Contains(EmptyCollectionCriteria) Then Criteria.Operands.Remove(EmptyCollectionCriteria)
            Dim args As SearchStartEventArgs = New SearchStartEventArgs(Criteria)
            OnSearchStart(args)
            If Not args.Cancel Then
                searchResultsCore.SuspendChangedEvents()
                searchResultsCore.Criteria = args.Criteria
                searchResultsCore.LoadAsync(New Helpers.AsyncLoadObjectsCallback(AddressOf LoadSearchResultsCallback))
            End If
        End Sub

        Private Sub LoadSearchResultsCallback(ByVal result As ICollection(), ByVal exception As Exception)
            Try
                If exception IsNot Nothing Then Throw New InvalidOperationException(String.Format("Cannot load search results for the {0} object type due to an error: {1}", GetType(T), exception.Message))
            Finally
